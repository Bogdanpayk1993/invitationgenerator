...
router.post('/getInvitations', async function (req, res) {
    ...
    res.send({ folder_name: req['body']['folder_name'], archive_name: `${req['body']['folder_name']}.zip` })
})

router.post('/deleteInvitations', upload.none(), async function (req, res) {
    res.end()

    let folder_names = JSON.parse(req['body']['folder_names'])

    folder_names.forEach(el => {
        fs.rm(`${path}\\public\\images\\invitations\\${el}`, { recursive: true, force: true }, err => {
            if (err) {
